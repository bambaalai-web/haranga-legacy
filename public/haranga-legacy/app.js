(async function(){
  // detect edition from path: /haranga-legacy/editions/0001/
  ...
  async function refresh(){
    ...
    if (data.owner){
      elOwner.textContent = data.owner;
      elClaim.hidden = true;
      elStatus.innerHTML = `<span class="lock">Locked</span> · Claimed at ${new Date(data.claimed_at).toLocaleString()}`;
    }else{
      elOwner.textContent = '(unclaimed)';
      elClaim.hidden = false;
      elStatus.textContent = 'First claim will lock this edition.';
    }
  }
  ...
  if (elForm) {
    elForm.addEventListener('submit', async (e)=>{
      ...
      const r = await fetch(`/api/edition/${edition}`, {
        method:'POST',
  ...
  if (audio) {
    ...
    // Шууд тоглуулахыг оролдох
    tryPlay();
    // Хэрэв хориглосон бол эхний click/tap дээр тоглуулна
